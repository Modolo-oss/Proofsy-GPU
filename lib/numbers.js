...
export async function commitJobReceipt(jobData) {
  ...
  formData.append('caption', `GPU Job ${jobData.eventType}: ${jobData.taskType}`);
  formData.append('headline', `ProofsyGPU - ${jobData.taskType} Job Receipt`);
  formData.append('tag', `gpu-job,${jobData.taskType},${jobData.eventType.toLowerCase()}`);
  ...
  // Add job metadata as separate fields for better blockchain storage
  if (jobData.metadata.jobId) formData.append('jobId', jobData.metadata.jobId);
  if (jobData.metadata.taskType) formData.append('taskType', jobData.metadata.taskType);
  if (jobData.metadata.taskName) formData.append('taskName', jobData.metadata.taskName);
  if (jobData.metadata.gpuType) formData.append('gpuType', jobData.metadata.gpuType);
  if (jobData.metadata.estimatedDuration) formData.append('estimatedDuration', jobData.metadata.estimatedDuration);
  if (jobData.metadata.inputHash) formData.append('inputHash', jobData.metadata.inputHash);
  if (jobData.metadata.executor) formData.append('executor', jobData.metadata.executor);
  if (jobData.metadata.status) formData.append('status', jobData.metadata.status);
